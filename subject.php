...
<div class="max-width" style="background-color: var(--w); position: absolute; left: 20%;">
    <div class="top" style="display: flex; flex-direction: row; justify-content: space-between; align-items: center;">
        ...
        <div class="right">
            <a class="lecturerBtn" href='admindashboard.php?info=add_subject'>
...
if ($result->num_rows > 0) {
  ...
  while($row = $result->fetch_assoc()) {
	...
	$sql3 = "SELECT name FROM teacher where teacher_id='".$row['teacher_id']."'";
	$result3 = $con->query($sql3);
	$row3 = $result3->fetch_assoc();
  	echo "<td>".$row3["name"]."</td>";

  	echo "<td>".$row["lecture_per_week"]."</td>";
  	echo "<td>".$row["type"]."</td>";
  	
            echo "<td>
                <abbr title='update'>
                    <a href='admindashboard.php?info=updatesubject&subject_id=".$row["subject_id"]."'>
            ...
            echo"<td>
                <abbr title='delete'>
                    <a href='javascript:deleteData(".$row["subject_id"].")'>
                        <svg width='23' height='23' viewBox='0 0 15 15' fill='none' xmlns='http://www.w3.org/2000/svg'
